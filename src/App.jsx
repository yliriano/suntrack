...
function App() {
  ...
  const canExport = showLiveData && !loading && displayData.length > 0;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="bg-slate-900 sticky top-0 z-10 shadow-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => exportToCSV(displayData, leadStatuses)} disabled={!canExport} className="inline-flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-lg bg-amber-500 text-slate-900 hover:bg-amber-400 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-md">
      ...
      <div className="bg-gradient-to-b from-slate-900 to-slate-800 px-4 sm:px-6 pt-6 pb-8">
        <div className="max-w-7xl mx-auto"><Filters filters={filters} onChange={setFilters} /></div>
      </div>
      ...
      <footer className="bg-slate-900 border-t border-slate-700/50 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
            ...
            {[{label:'DC',url:'https://opendata.dc.gov/'},{label:'NJ Statewide',url:'https://data.nj.gov/Reference-Data/NJ-Construction-Permit-Data/w9se-dmra'},{label:'Austin',url:'https://data.austintexas.gov/Building-and-Development/Issued-Construction-Permits/3syk-w9eu'},{label:'Dallas',url:'https://www.dallasopendata.com/Services/Building-Permits/e7gq-4sah'},{label:'SF',url:'https://data.sfgov.org/Housing-and-Buildings/Building-Permits/i98e-djp9'},{label:'Los Angeles',url:'https://data.lacity.org/City-Infrastructure-Service-Requests/Building-and-Safety-Electrical-Permits-Issued-from/ysqd-apz7'},{label:'Cary NC',url:'https://data.townofcary.org/explore/dataset/solar-permit-applications/'},{label:'Chicago IL',url:'https://data.cityofchicago.org/Buildings/Building-Permits/ydr8-5enu'},{label:'Fort Collins CO',url:'https://opendata.fcgov.com/Sustainability/Fort-Collins-Solar-Installations/3ku5-x4k9'}].map(({label,url})=>(<a key={label} href={url} target="_blank" rel="noopener noreferrer" className="hover:text-amber-400 transition-colors">{label}</a>))}
          </div>
        </div>
      </footer>
