...
const APIS = {
  ...
  nc_cary: { base:'https://data.townofcary.org/api/explore/v2.1/catalog/datasets/solar-permit-applications/records', isCary:true, normalize:(p)=>({_source:'nc_cary',_sourceLabel:'Cary Open Data',_sourceUrl:'https://data.townofcary.org/explore/dataset/solar-permit-applications/',permit_number:p.permitnum||p.permit_number||'',application_date:p.applieddate||p.application_date||'',issue_date:p.issuedate||p.issue_date||'',address_1:p.originaladdress1||p.site_address||p.address||'',permit_type_name:p.permit_type||p.type||'Solar Permit',permit_subtype_name:p.description||'',status_current:p.statuscurrent||p.statuscurrentmapped||p.status||'',contractor_company_name:p.contractorcompanyname||p.contractor_company||'',contractor_trade:p.contractortrade||p.contractortrademapped||'',owner_name:p.ownername||p.owner||'',municipality:p.jurisdiction||'Cary',state:'NC'}) },
      ...
      issue_date: p.date_of_service || '',
      address_1: p.system_address || '',
      permit_type_name: 'Solar Installation',
      permit_subtype_name: p.system_capacity_kw_dc ? `${p.system_capacity_kw_dc} kW DC` : '',
      status_current: 'Installed',
  ...
  // California — Los Angeles Electrical Permits (Socrata — solar='Y' flag)
  ca_la: {
    base: 'https://data.lacity.org/resource/ysqd-apz7.json',
    solarWhere: "solar='Y'",
    orderField: 'issue_date',
    normalize: (p) => ({
      _source: 'ca_la',
      _sourceLabel: 'LA Open Data',
      _sourceUrl: 'https://data.lacity.org/City-Infrastructure-Service-Requests/Building-and-Safety-Electrical-Permits-Issued-from/ysqd-apz7',
      permit_number: p.permit_nbr || '',
      application_date: p.submitted_date || p.issue_date || '',
      issue_date: p.issue_date || '',
      address_1: p.primary_address || '',
      permit_type_name: p.permit_group || p.permit_type || '',
      permit_subtype_name: p.work_desc || p.permit_sub_type || '',
  ...
  // North Carolina — Town of Cary (OpenDataSoft — dedicated solar dataset)
  nc_cary: {
    base: 'https://data.townofcary.org/api/explore/v2.1/catalog/datasets/solar-permit-applications/records',
    isCary: true, // OpenDataSoft uses different query params
    normalize: (p) => ({
      _source: 'nc_cary',
      _sourceLabel: 'Cary Open Data',
      _sourceUrl: 'https://data.townofcary.org/explore/dataset/solar-permit-applications/',
      permit_number: p.permitnum || p.permit_number || '',
      application_date: p.applieddate || p.application_date || '',
      issue_date: p.issuedate || p.issue_date || '',
      address_1: p.originaladdress1 || p.site_address || p.address || '',
      permit_type_name: p.permit_type || p.type || 'Solar Permit',
      permit_subtype_name: p.description || '',
      status_current: p.statuscurrent || p.statuscurrentmapped || p.status || '',
      contractor_company_name: p.contractorcompanyname || p.contractor_company || '',
      contractor_trade: p.contractortrade || p.contractortrademapped || '',
      owner_name: p.ownername || p.owner || '',
      municipality: p.jurisdiction || 'Cary',
      state: 'NC',
    }),
  },
};

// Which APIS to call per jurisdiction selection
const JURISDICTION_APIS = {
  ...
  co:  ['co_fc'],
  ks:  [],
  all: ['dc', 'nj', 'tx_austin', 'tx_dallas', 'ca_sf', 'ca_la', 'nc_cary', 'il_chicago', 'co_fc'],
};

// Portal-only jurisdictions — no API calls needed
const PORTAL_ONLY_JURISDICTIONS = new Set(['fl', 'az', 'montgomery', 'pgcounty', 'ks']);

// Fetch helpers

const fetchSocrata = async (config) => {
  const query = { $limit: '5000', $order: `${config.orderField} DESC` };
  if (config.solarWhere) query['$where'] = config.solarWhere;
  const params = new URLSearchParams(query);
  const res = await fetch(`${config.base}?${params}`);
  if (!res.ok) throw new Error(`${config.base} → ${res.status}`);
  const data = await res.json();
  return (Array.isArray(data) ? data : []).map(config.normalize);
};

const fetchCary = async (config) => {
  const PAGE_SIZE = 100;
  const base = `${config.base}?limit=${PAGE_SIZE}&order_by=applieddate+desc`;

  // First page — also reveals total_count
  const firstRes = await fetch(`${base}&offset=0`);
  if (!firstRes.ok) throw new Error(`Cary API → ${firstRes.status}`);
  const firstJson = await firstRes.json();
  const total = firstJson.total_count || 0;
  const firstPage = firstJson.results || [];

  const numPages = Math.ceil(total / PAGE_SIZE);
  let allRecords = firstPage;

  if (numPages > 1) {
    const offsets = Array.from({ length: numPages - 1 }, (_, i) => (i + 1) * PAGE_SIZE);
    ...
    allRecords = [firstPage, ...rest].flat();
  }

  return allRecords.map((r) => config.normalize(r.fields || r));
};

const fetchSource = (key) => {
  const config = APIS[key];
  if (!config) return Promise.resolve([]);
  return config.isCary ? fetchCary(config) : fetchSocrata(config);
};
